...
class Markdown:
    ...
    def __init__(self, store: wasmtime.Store, linker: wasmtime.Linker, module: wasmtime.Module):
        ...
        assert(isinstance(render, wasmtime.Func))
        self._render = render
    def render(self, caller: wasmtime.Store, input: str) -> str:
        memory = self._memory;
        realloc = self._canonical_abi_realloc
        free = self._canonical_abi_free
        ptr, len0 = _encode_utf8(input, realloc, memory, caller)
        ret = self._render(caller, ptr, len0)
        ...
        return list
    def overhead(self, caller: wasmtime.Store, input: str) -> Tuple[str, int]:
        memory = self._memory;
        realloc = self._canonical_abi_realloc
        free = self._canonical_abi_free
        ptr, len0 = _encode_utf8(input, realloc, memory, caller)
